...
if st.session_state["active_tab"] == 0:
    ...
    if not st.session_state["quiz_submitted"]:
        ...
        if st.session_state["portfolio_type"] == "general":
            ...
            st.markdown('<div class="soft-card" style="margin-bottom:1rem;"><h4>Build Your Own Inputs</h4><p>Enter the expected return, risk, and ESG score for each asset, then set the relationship between them using correlation and the risk-free rate.</p></div>', unsafe_allow_html=True)
            a_col1, a_col2 = st.columns(2)
            with a_col1:
                st.markdown('<div class="soft-card" style="margin-bottom:0.9rem;"><h4>Asset 1</h4><p>Start with the more defensive or familiar asset first so the comparison feels easier to reason about.</p></div>', unsafe_allow_html=True)
            ...
            with a_col2:
                st.markdown('<div class="soft-card" style="margin-bottom:0.9rem;"><h4>Asset 2</h4><p>Use this to compare against a higher-growth, lower-volatility, or more sustainable alternative.</p></div>', unsafe_allow_html=True)
            ...
            st.markdown('<div class="soft-card" style="margin:1rem 0 0.8rem;"><h4>Market Assumptions</h4><p>Correlation controls how closely the two assets move together. The risk-free rate sets the baseline return for the portfolio optimisation.</p></div>', unsafe_allow_html=True)
    ...
    if results is None:
        ...
        fig1.add_trace(go.Scatter(x=[round(tan["sd"]*100,2)], y=[round(tan["ret"]*100,2)], mode="markers", name="Tangency Portfolio", marker=dict(color=PLOTLY_COLORS["tangency"], size=18, symbol="diamond", line=dict(color=PLOTLY_COLORS["outline"], width=2))))
        fig1.add_trace(go.Scatter(x=[round(opt["sd"]*100,2)], y=[round(opt["ret"]*100,2)], mode="markers", name="ESG-Optimal", marker=dict(color=PLOTLY_COLORS["esg_opt"], size=22, symbol="star", line=dict(color=PLOTLY_COLORS["outline"], width=2))))
        fig1.add_trace(go.Scatter(x=[round(std_r["sd"]*100,2)], y=[round(std_r["ret"]*100,2)], mode="markers", name="Standard Optimal", marker=dict(color=PLOTLY_COLORS["std_opt"], size=18, symbol="circle", line=dict(color=PLOTLY_COLORS["outline"], width=2))))
        ...
        fig2.add_trace(go.Scatter(x=[round(opt["esg"]*10,2)], y=[round(opt_sharpe,4)], mode="markers", name="ESG-Optimal", marker=dict(color=PLOTLY_COLORS["esg_opt"], size=22, symbol="star", line=dict(color=PLOTLY_COLORS["outline"], width=2))))
        fig2.add_trace(go.Scatter(x=[round(std_r["esg"]*10,2)], y=[round(std_sharpe,4)], mode="markers", name="Standard Optimal", marker=dict(color=PLOTLY_COLORS["std_opt"], size=18, symbol="circle", line=dict(color=PLOTLY_COLORS["outline"], width=2))))
